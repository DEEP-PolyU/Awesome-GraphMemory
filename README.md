...
- [![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/mnemoverse/mcp-memory-server) Mnemoverse: Managed persistent memory over MCP with a learned associative layer. Concepts are linked by a three-factor Hebbian rule tuned by a Rescorla-Wagner update; edges carry no relation types and no entity resolution, so the layer never asserts that two records denote the same thing. Consolidation clusters similar memories with HDBSCAN under Von Restorff protection, and reported outcomes re-rank later recall. MIT MCP server and Python SDK, managed engine. Retrieval model published as [SLoD](https://arxiv.org/abs/2603.08965).
