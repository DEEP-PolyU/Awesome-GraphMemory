...
- [![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/jaylfc/taosmd) taOSmd: Local-first, benchmarked agent memory on an append-only, zero-loss transcript. A typed temporal knowledge graph supersedes corrected facts via invalidation, and each fact links back to its source span so a verifier can demote ones it cannot support before they are served. Hybrid vector + BM25 retrieval tuned for small local models; runs fully offline on an 8GB SBC. 97% Recall@5 on LongMemEval-S, reproducible. MIT.
